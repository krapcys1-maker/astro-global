...
- Article draft harness laduje teraz `.env` i mapuje `DEEPSEEK_API_KEY` na live config z domyslnym `DEEPSEEK_BASE_URL=https://api.deepseek.com/chat/completions` oraz `DEEPSEEK_MODEL=deepseek-v4-flash`.
- Pierwszy live run DeepSeek dla `article_revolutionary_wave_1789_1848` przeszedl: provider `deepseek-v4-flash`, `validation_ok=true`; live draft i osobna walidacja zapisane w `work/reports/`.
- Manualny review pierwszego live draftu wykazal, ze walidator techniczny dziala, ale prompt byl za szeroki: model dodawal encyklopedyczne interpretacje spoza fact-packa. Prompt zostal zaostrzony do stylu evidence-ledger.
- Drugi live draft po zaostrzeniu promptu przeszedl walidacje (`ok=true`) i jest bardziej zwiazany z backend fact-packiem: raportuje similarity, epizody, matched events i context event bez szerokiego dopowiadania przyczyn/skutkow. Raport review: `work/reports/article_draft_live_review_revolutionary_wave_1789_1848.md`.
...
2. Nastepny krok AI: odpalic live draft na drugim seedzie (`article_modern_crisis_2019_2022` albo `article_world_wars_1914_1939`) i sprawdzic, czy evidence-ledger prompt utrzymuje jakosc poza przypadkiem 1789/1848.
...
- 2026-05-23: Podpieto `web/` z powrotem do realnego FastAPI jako cienki klient: `/today`, `/resonance/search`, `/resonance/compare`, `/timeline/seeds` i `/articles/seeds`; Explorer renderuje backendowe matched/context events, score breakdown, confidence i source links bez fake danych.
...
- 2026-05-25: Dopracowano topbar/navbar Explorera pod referencje premium institutional/trading-platform: szerszy brand lockup, gestszy rytm tabow, spokojniejszy aktywny `Overview`, subtelniejsze separatory, matowe tlo i zachowany format daty `dd-mm-yyyy`.
- 2026-05-25: Weryfikacja po korekcie topbara: `python scripts/smoke_test_web_shell.py` przechodzi, `python scripts/smoke_test_web_api_e2e.py` przechodzi, a zrzuty porownawcze zapisano w `work/reports/screenshots/topbar_before_20260525.png` i `work/reports/screenshots/topbar_after_final_20260525.png`.
- 2026-05-25: Wyrownano srodkowy rzad Explorera: `Most Similar Historical Regimes` i `Cycle Layers` maja teraz wspolna wysokosc, mala szczeline jak w referencji oraz wiecej realnej szerokosci dla kart analogii bez pustego pasa miedzy panelami.
- 2026-05-25: Wzmocniono renderowanie dat przy `Key events` w kartach analogii: UI uzywa `display_date`, a gdy go zabraknie, sklada etykiete z `start_astro_year/end_astro_year`; odswiezono cache `shell.js` i zapisano screenshot `work/reports/screenshots/key_events_dates_fallback_20260525.png`.
- 2026-05-25: Ujednolicono daty cykli w Explorerze: dominujace cykle i `Cycle Layers` pokazuja pelne daty start/end w formacie `dd-mm-yyyy` takze dla wieloletnich zakresow, zamiast skracac je do samych lat.
- 2026-05-25: Po pelnym porownaniu z referencja dopracowano proporcje Explorera: szersza kolumna cykli w `Current Regime`, czytelniejsze pelne daty cykli, cichsze etykiety kart analogii, bardziej matowe miniwykresy oraz lepiej wypelnione karty `Cycle Layers`; screenshoty: `full_compare_before_20260525.png` i `full_compare_after_polish_20260525.png`.
- 2026-05-25: Naprawiono walidacje wydarzen w kartach `Historical Analogues`: `matched_events` wymaga teraz precyzyjnej daty albo dokladnego zakresu przecinajacego `Exact resonance window`, szerokie/roczne wydarzenia trafiaja do `context_events` z `temporal_precision`, `temporal_relation` i `temporal_match_score`, a UI rozdziela `Exact window events` od `Broader historical context` zamiast mieszac je jako `Key events`.
- 2026-05-26: Poprawiono semantyke dat w kartach `Historical Analogues`: glowny naglowek karty jest teraz `Resonance period` i wynika z tego samego `period_start/period_end` co `Exact resonance window`; lata wydarzen kontekstowych nie sa juz uzywane jako duzy zakres analogii.
- 2026-05-26: Dodano `resonance_basis` do epizodow `/resonance/search`: kazda analogia pokazuje teraz realne drivery `Now` i `Then` wyliczone z pozycji wolnych planet oraz cykli dla daty zapytania i historycznego `best_date`; karty Explorera renderuja te dane jako `Resonance basis`, zeby bylo jasne, co z czym jest porownywane.
- 2026-05-26: Naprawiono zakres wyszukiwania dla `historical_analogue_mode`: persistent index `swiss_1500_now_global_slow_v1.npz` nie jest juz przycinany do `lookback_years/lookahead_years` przed szukaniem analogii, wiec Explorer moze realnie przeszukiwac pelny indeks 1500-now; dodano test regresji i zweryfikowano request 2026-05-25 na `27 466` wierszach indeksu.
- 2026-05-26: Przeprojektowano wylacznie gorna nawigacje/toolbar web shell: usunieto techniczne zakladki Explorera (`Epoch Background`, `Structural Cycles`, `Short-Term Cycles`, `Historical Analogues`, `Events Timeline`, `Map View`, `Settings`) z topbara i zastapiono je polska struktura produktu `Start`, `Dzis`, `Kosmogram`, `Cykle historyczne`, `Porownania`, `Blog`, `O silniku`, `Kontakt`, `Logowanie`; zachowano backend i layout kart/timeline/cycle layers bez zmian.
- 2026-05-26: Zdiagnozowano i naprawiono over-scoring warstwy planetary resonance dla 2001 -> early 1500s: surowy cosine nagradzal ciagla geometrie faz i sasiednie znaki zewnetrznych planet, wiec 1503 mogl dostac ~89% mimo braku zgodnego srodowiska Uran/Neptun/Pluton; dodano kalibracje `calibrate_structural_similarity`, reranking kandydatow po score skorygowanym oraz testy regresji dla mismatchu znakow, wspolnego aspektu outer-planet i przypadku 2001-05-06 vs 1503-03-23. Weryfikacja: `pytest`, `ruff`, goldeny API i OpenAPI snapshot check przechodza.
- 2026-05-26: Ustabilizowano top navigation web shell: `Cykle historyczne`, `Porownania`, `Logowanie` i pozostale widoki maja teraz centrowany tor nawigacji, aktywny stan nie zmienia geometrii linku, `Logowanie` nie wyglada jak drugi aktywny element, a Explorer i publiczny header nie przesuwaja paska miedzy widokami. Weryfikacja: `smoke_test_web_shell.py`, `smoke_test_web_api_e2e.py` oraz manualny browser check pozycji nav.
- 2026-05-26: Naprawiono dwa problemy po kliknieciu `Cykle historyczne`: osobny topbar Explorera zostal obnizony z 88px do 74px i wyrownany pionowo z reszta toolbaru, a requesty frontend -> FastAPI dostaly timeout 20s, zeby Explorer nie zostawal na wiecznym loaderze przy niedostepnym backendzie. Browser check potwierdzil render pelnego `Current Regime` dla `24-05-2026` po wejsciu w `/cykle-historyczne`.
- 2026-05-26: Usunieto lewy rail Explorera (`Explorer`, `Compare`, `Calendar`, `Cycles`, `Library`, `Reports`) jako niepotrzebna pozostalosc dashboardowa; Explorer ma teraz prosty grid `top/main` i wykorzystuje pelna szerokosc strony. Browser check potwierdzil `railExists=false`, `topbarHeight=74` i render `reference-regime-panel`.
- 2026-05-26: Dodano warstwe diversity selection dla finalnych kart `/resonance/search`: raw kandydaci pozostaja w `raw_historical_candidates`, a UI-facing epizody grupuja bliskie okna przez `related_windows` zamiast pokazywac obok siebie kilka punktow z tego samego lokalnego maksimum. Reguly obejmuja 5-letni cooldown, overlap eventow >60% i sasiednie dlugie procesy historyczne; targeted Swiss search dla 2001/2000, 1977-1985 i 1849-1852 potwierdzil grupowanie klastrow 1500s, 1810s/1820s, 1950s/1990s bez zmiany planetary scoringu.
- 2026-05-26: Naprawiono start `Cykle historyczne`: Explorer nie uzywa juz starej daty fallbackowej, tylko date z `/today` albo lokalne dzisiaj, czysci stary wynik przy wejsciu w zakladke i automatycznie odpala `/resonance/search` bez recznego wpisywania daty. Odwiezono cache-bust `shell.js`; browser check na aktualnym backendzie potwierdzil `inputValue=26-05-2026` i render `reference-regime-panel`.
- 2026-05-26: Dodano frontendowy guardrail dla kart `Most similar historical regimes`: nawet jesli starszy/stale backend zwroci sasiednie okna typu 1526/1527/1528 jako osobne epizody, web shell grupuje je w jeden okres/kartę i dopina sasiednie okna do `related_windows`; naglowek karty potrafi pokazac zakres klastra, np. `1526-1528`. Weryfikacja: web shell smoke, web/API E2E smoke i `ruff`.
- 2026-05-26: Zaostrzono wiarygodnosc historycznego kontekstu analogii: ultra-szerokie procesy ponad ok. 50 lat, np. `Atlantic slave trade` albo wielowiekowe ruchy, nie przechodza juz jako `context_events` dla waskiego piku rezonansu; approximate/open-ended events dostaly duration penalty, a deduplikacja analogii nie tlumi juz odleglych epok tylko dlatego, ze dziela te same szerokie eventy. Karty UI zmienily etykiety na `Historical resonance cluster` i `Strongest similarity peak` oraz pokazuja realny composite score i wklady glownych cykli z `resonance_basis`.
- 2026-05-26: Wzmocniono wiarygodnosc analogii: diversity selection rozpoznaje teraz pobliskie makro-struktury rezonansu przez overlap driverow planetarnych, score breakdown pokazuje realne skladowe (`aspect/vector`, `active-cycle power`, `rarity`, guardraile znakow/aspektow), a kontekst historyczny dla klastrow pobiera precyzyjne fazy/inflection points z okna klastra bez rozciagania na dalekie suppressed macro windows. Dodano sourced events `Sack of Rome 1527` i `Augsburg Confession 1530`; weryfikacja: `pytest`, `ruff`, `validate_curated_data`, goldeny/OpenAPI, smoke testy web oraz browser check `/cykle-historyczne`.
- 2026-05-26: Dodano modul `Kosmogram` / natal chart: backend `/natal-chart/calculate` liczy realne pozycje planet, domy, osie ASC/DSC/MC/IC, aspekty i dominanty przez Swiss Ephemeris, `/locations/search` obsluguje lokalny katalog miast oraz manualne koordynaty/timezone, a frontend renderuje premium dark SVG natal wheel z tabelami planet, domow, aspektow, dominant i placeholderem interpretacji. Dodano testy geometrii kola, osi, domow, aspektow, unknown-time mode i API; weryfikacja: `pytest`, `ruff`, OpenAPI check, web shell smoke, web/API E2E smoke oraz browser check strony `/kosmogram`.
