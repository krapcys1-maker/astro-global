...
DEFAULT_OUTPUT = (
    ROOT / "tests" / "golden" / "resonance_compare" / "presets_swiss_1500_now.json"
)
GOLDEN_SESSION_TOKEN = "golden-test-token"
GOLDEN_AUTH_HEADERS = {"x-astro-global-session": GOLDEN_SESSION_TOKEN}


def build_resonance_compare_presets_golden() -> dict:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Update or verify the golden /resonance/compare/presets API response."
    )
    ...
    rendered = serialize(build_resonance_compare_presets_golden())
    if args.check:
        existing = args.output.read_text(encoding="utf-8")
        if existing != rendered:
            raise SystemExit(
                f"{args.output} is out of date. "
                "Run `python scripts/update_resonance_compare_presets_golden.py`."
            )
