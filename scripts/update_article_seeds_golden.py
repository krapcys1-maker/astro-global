...
DEFAULT_OUTPUT = ROOT / "tests" / "golden" / "articles" / "seeds_swiss_1500_now.json"
GOLDEN_SESSION_TOKEN = "golden-test-token"
GOLDEN_AUTH_HEADERS = {"x-astro-global-session": GOLDEN_SESSION_TOKEN}


def build_article_seeds_golden() -> dict:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Update or verify the golden /articles/seeds API response."
    )
    ...
    rendered = serialize(build_article_seeds_golden())
    if args.check:
        existing = args.output.read_text(encoding="utf-8")
        if existing != rendered:
            raise SystemExit(
                f"{args.output} is out of date. "
                "Run `python scripts/update_article_seeds_golden.py`."
            )
